...
// Map Motives
if (masterData['motives']) {
  masterData['motives'].forEach(item => {
    let baseName = item.name.replace(/\s*\(.*?\)\s*/g, '').trim();
    if (motiveDictionary[item.name] || motiveDictionary[baseName]) {
      item.icon = motiveDictionary[item.name] || motiveDictionary[baseName];
    } else {
      // Fallback to something generic if not found
      item.icon = "fa-solid fa-circle-question";
    }
  });
}

fs.writeFileSync(dataPath, JSON.stringify(masterData, null, 2), 'utf-8');
console.log(`✅ Update complete! Fixed items and mapped motives successfully!`);
